...
def main():
    ...
    # Train a regrssion model using different feature sets
    feature_array = [traditional_features,  telematics_features, all_feature_cols]

    # Initialize a logger to keep track of training and evaluation status
    logger = initialize_training_logger(model_name="training_all_regressors")


    for feat_arr in feature_array:
        feature_cols = feat_arr
        ...
        if feature_cols == traditional_features:
            ...
            logger.info("Using telematics features only")
            model_name = "telematics"
        else:
            print("Using traditional + telematics features")
            logger.info("Using traditional + telematics features")
        ...
        #  Attach risk scores back to dataset
        ...
        preprocessed_df.to_csv(f"data/preprocessed_data/processed_dataset_{model_name}_with_risk_scores_10000.csv")
        logger.info(f"Dataset with risk scores and premiums saved to data/preprocessed_data/processed_dataset_{model_name}_with_risk_scores_10000.csv")
        print(f"Dataset with risk scores and premiums saved to data/preprocessed_data/processed_dataset_{model_name}_with_risk_scores_10000.csv")
